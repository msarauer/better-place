...
module.exports = (db) => {
  ...
  router.put("/:user/:reviewid", (req,res) => {
    const data = req.body;
    db.query(`UPDATE volunteer_reviews SET (user_feedback, rating) = ($1, $2) WHERE user_id = $3 AND volunteer_reviews.id = $4 RETURNING *`,
  ...
  router.delete("/:id", (req, res) => {
    db.query(`DELETE FROM volunteer_reviews WHERE volunteer_reviews = $1 RETURNING *`, [req.params.id])
    .then((data) => {
      const review = data.rows;
      res.json({ review })
    })
